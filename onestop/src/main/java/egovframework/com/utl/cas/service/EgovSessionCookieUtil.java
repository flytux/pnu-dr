...
public class EgovSessionCookieUtil {
	...
	HttpSession session = request.getSession();
	String returnVal = "{sessionid : " + session.getId() + "},";
     ...
     */
    @SuppressWarnings("unchecked")
	public static String getSessionIDValuesString(HttpServletRequest request) throws Exception {
	HttpSession session = request.getSession();
	String returnVal = session.getId();	
	return returnVal;
    }
	...
	/** 시큐어 코딩 : 최대값 60분을 넘지 않도록 세팅 */
	if (minute2 > 60) minute2 = 60;
	// 쿠키의 유효시간 설정
	cookie.setMaxAge(60 * minute2);
