...
$.datepicker.regional['ko'] = {
	...
	buttonText: "<i class='bi bi-calendar-event'><span class='sr-only'>달력열기</span>",
	prevText: '이전',
	nextText: '다음',
	currentText: '오늘',
...
$.fn.setValue = function(val, callback) {
	if ($(this).next().find('.select-pure__option[data-value="' + val + '"]').length > 0) {
		...
		if (gfn_str_parseNull(val) != "") {
			$.each(val_str.split(","), function(idx, item) {
				_obj._setValue(item);
			});
		}
		$(this).val(gfn_str_replaceAll(val_str, " ", ""));
		$(this).trigger("change");
...
function gfn_com_assignCombo(options) {
	...
	if($('#langGcd').val()=="0001"){
		var all1 = gfn_str_parseNull(options.all1) == "Y" ? "전체" : "선택";
		var all2 = gfn_str_parseNull(options.all2) == "Y" ? "전체" : "선택";
		var all3 = gfn_str_parseNull(options.all3) == "Y" ? "전체" : "선택";
		var all4 = gfn_str_parseNull(options.all4) == "Y" ? "전체" : "선택";
		var all5 = gfn_str_parseNull(options.all5) == "Y" ? "전체" : "선택";
	} else {
		var all1 = gfn_str_parseNull(options.all1) == "Y" ? "All" : "Select";
		var all2 = gfn_str_parseNull(options.all2) == "Y" ? "All" : "Select";
		var all3 = gfn_str_parseNull(options.all3) == "Y" ? "All" : "Select";
		var all4 = gfn_str_parseNull(options.all4) == "Y" ? "All" : "Select";
		var all5 = gfn_str_parseNull(options.all5) == "Y" ? "All" : "Select";
	}
		...
		if (options.comboLvl > 0) {
			scwin[$(options.collGradGcd).attr("id") + "_multi"] = $(options.collGradGcd).extDropDownList({
				...
				dataBound: function(e) {
					//$(options.collGradGcd).data(e);
					$.each($("#" + $(options.collGradGcd).attr("id") + "_multi .select-pure__options .select-pure__option"), function(idx, item) {
						$(this).attr("tabindex","0");
					});
					$("#" + $(options.collGradGcd).attr("id") + "_multi .select-pure__options .select-pure__option--selected").removeAttr("tabindex");
					$("#" + $(options.collGradGcd).attr("id") + "_multi").prepend('<span class="sr-only">대학/대학원을 선택해주세요.</span>')
					setTimeout(function() {
							$(options.collGradGcd).focus(function(e){
									$("#"+$(options.collGradGcd).attr("id")+"_multi").find(".select-pure__select").focus();
							});
					}, 500);
					...
					if (options.comboLvl > 1) {
						gfn_ajax_request({
							...
							, success: function(data, responseData) {
								if(gfn_str_parseNull(options.all2) == "Y") {
									data[0].label = all2;
								}
								...
								scwin[$(options.gradGcd).attr("id") + "_multi"] = $(options.gradGcd).extDropDownList({
									...
									dataBound: function(e) {
										...
										$("#" + $(options.gradGcd).attr("id") + "_multi").prepend('<span class="sr-only">대학원을 선택해주세요.</span>')
										...
										$("#" + $(options.gradGcd).attr("id") + "_multi  .select-pure__select").click(function(){

										});
										setTimeout(function() {
											$(options.gradGcd).focus(function(e){
												$("#"+$(options.gradGcd).attr("id")+"_multi").find(".select-pure__select").focus();
											});
										}, 500);
										$(options.collGradGcd).change(function() {
											...
											$.each($(options.gradGcd).data(), function(idx, item) {
												if (item.COLL_GRAD_GCD == $(options.collGradGcd).val()) {
													...
													$("#" + $(options.gradGcd).attr("id") + "_multi .select-pure__options .select-pure__option:eq(" + idx + ")").attr("tabindex","0");
										...
										if (options.comboLvl > 2) {
											gfn_ajax_request({
												...
												, success: function(data, responseData) {
													if(gfn_str_parseNull(options.all3) == "Y") {
														data[0].label = all3;
													}
													...
													(scwin[$(options.collCd).attr("id") + "_multi"]) = $(options.collCd).extDropDownList({
														...
														dataBound: function(e) {
															...
															$("#" + $(options.collCd).attr("id") + "_multi").prepend('<span class="sr-only">단과대학을 선택해주세요.</span>')
															...
															setTimeout(function() {
																$(options.collCd).focus(function(e){
																	$("#"+$(options.collCd).attr("id")+"_multi").find(".select-pure__select").focus();
																});
															}, 500);
															$(options.gradGcd).bind("change , keyup",function(e) {
																if(e.type =="change" || e.keyCode ==32 || e.keyCode== 13){
																	...
																	$.each($(options.collCd).data(), function(idx, item) {
																		if (item.COLL_GRAD_GCD == $(options.collGradGcd).val()
																			...
																			$("#" + $(options.collCd).attr("id") + "_multi .select-pure__options .select-pure__option:eq(" + idx + ")").attr("tabindex","0");
															...
															if (options.comboLvl > 3) {
																gfn_ajax_request({
																	...
																	, success: function(data, responseData) {
																		if(gfn_str_parseNull(options.all4) == "Y") {
																			data[0].label = all4;
																		}
																		...
																		(scwin[$(options.deptCd).attr("id") + "_multi"]) = $(options.deptCd).extDropDownList({
																			...
																			dataBound: function(e) {
																				...
																				$("#" + $(options.deptCd).attr("id") + "_multi").prepend('<span class="sr-only">학과(부)/전공을 선택해주세요.</span>')
																				...
																				setTimeout(function() {
																					$(options.deptCd).focus(function(e){
																						$("#"+$(options.deptCd).attr("id")+"_multi").find(".select-pure__select").focus();
																					});
																				}, 500);
																				...
																				$(options.collCd).change(function() {
																					...
																					$.each($(options.deptCd).data(), function(idx, item) {
																						if (item.COLL_GRAD_GCD == $(options.collGradGcd).val()
																							...
																							$("#" + $(options.deptCd).attr("id") + "_multi .select-pure__options .select-pure__option:eq(" + idx + ")").attr("tabindex","0");
																				...
																				if (options.comboLvl > 4) {
																					gfn_ajax_request({
																						...
																						, success: function(data, responseData) {
																							if(gfn_str_parseNull(options.all5) == "Y") {
																								data[0].label = all5;
																							}
																									...
																									$(options.deptCd).change(function() {
																										...
																										$.each($(options.majorCd).data(), function(idx, item) {
																											if (item.COLL_GRAD_GCD == $(options.collGradGcd).val()
																												...
																												$("#" + $(options.majorCd).attr("id") + "_multi .select-pure__options .select-pure__option:eq(" + idx + ")").attr("tabindex","0");
...
function gfn_com_assignCombo_dataBound(callback, _flag, options) {
	setTimeout(function() {
		...
		$(".select-pure__select").bind('click', function() {
			 $(".select-pure__select").not($(this)).removeClass('select-pure__select--opened');
		});
		$(".select-pure__select").bind('keyup', function(e) {
		...
		$(".select-pure__select").attr("tabindex","0");
		
		$(".select-pure__option").bind('keyup', function(e) {
							 var code = e.charCode || e.keyCode;
							
							if( (code== 32) || (code ==13)){
								$(".select-pure__select").not($(this)).removeClass('select-pure__select--opened');
								$(this).closest(".select-pure__select").find(".select-pure__label").html($(this).html())
								$(this).addClass("select-pure__option--selected");	
								
								$(this).closest(".select-pure__select").removeClass('select-pure__select--opened');
								$(this).closest(".select-pure__select").addClass('cls');
								$(".select-pure__option").not($(this)).removeClass('select-pure__option--selected');
								$(this).closest(".select-pure__select").parent().prev().setValue($(this).data("value"));
								//debugger;
							}
...
function gfn_com_maxZindex(startIdx) {
	...
	var ret_zindex = parseInt(num) + parseInt(startIdx);
	if(2147400000 < parseInt(ret_zindex)){
		ret_zindex = parseInt(ret_zindex) - 2147400000;
	}
	return ret_zindex;
}
...
function gfn_com_noticeModalPopup(params, callback) {
		...
		if (params.title) {
			...
			if ((params.id).indexOf('noticePopup') > -1) {
				if (params.move && params.move == "off") {
					$(divPopTag).append($("<nav class='_navbar _navbar-default'><div class='_container'><h1 class='_navbar-brand' style='width:auto;'><span data-lang >" + params.title + "</span></h1></div></nav>"));
				} else {
					$(divPopTag).append($("<nav class='_navbar _navbar-default' onmousedown='fn_startDrag(event, this)'><div class='_container'><h1 class='_navbar-brand' style='width:auto;'><span data-lang >" + params.title + "</span></h1></div></nav>"));
				}
				...
				if(gfn_str_parseNull(params.linkMenuCd) != ""){
					$(divPopTag).append("<div class='modal_link' style='padding:12px'><a href='#' onclick='selectMenu(\""+params.linkMenuCd+"\");return false;' onkeypress='this.onclick' class='btn btn-light d-block mt-1 p-3' >관련 페이지로 이동</a></div>");	
				}
		...
		if (params.overflow_y && params.overflow_y == "off") {
			...
			$(window).resize(function(e) {
				//pop_resizing(_width, suffixStr, params, _height, _top, _left);
				pop_resizing_notice(suffixStr, params);
			});
			loadscript($(divPopWrapTag), "/common/bootstrap-table-master/dist/bootstrap-table.min.js");
			$(".numeric").numeric();
			loadAccessibility();
...
function pop_resizing_notice(suffixStr, params) {
	$("#popupWrap" + suffixStr).css('height', params.height);
	if ($("#popupWrap" + suffixStr + " ._footer").length > 0) {
		$("#popupWrap" + suffixStr + " ._container:eq(1)").css('height', params.height - 100);
	}
	
	if ((params.width + 40) > $(window).width()) {	// 브라우저보다 팝업넓이가 크면 브라우저넓이로 팝업넓이를 맞춤
		$("#popupWrap" + suffixStr).css('width', $(window).width() - 40);
		$("#popupWrap" + suffixStr).css("top", "20px");
		$("#popupWrap" + suffixStr).css("left", "20px");
	} 
	else {	// 아니면 설정된 팝업넓이로 세팅
		$("#popupWrap" + suffixStr).css('width', params.width);
		
		if(params.position.center) {
			var _top = (($(window).height() - params.height) / 2 + $('html, body').scrollTop());
			var _left = (($(window).width() - params.width) / 2);
		
			$("#popupWrap" + suffixStr).css("top", _top + "px");
			$("#popupWrap" + suffixStr).css("left", _left + "px");
		}else {
			$("#popupWrap" + suffixStr).css("top", params.position.top + "px");
			$("#popupWrap" + suffixStr).css("left", params.position.left + "px");
		}
	}
}
...
function gfn_com_createDatetimeComp(compID, _format, _flag , callback) {
	$("#" + compID).wrap('<div class="input-group"></div>');
	...
	$("#" + compID).after('<div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-calendar-event"><span class="sr-only">달력열기</span></i></button></div>');
	$("#" + compID).parent().find("button").click(function(e) {
...
function gfn_com_createProfSearchComp(compID) {
	...
	$("#" + compID).parent().append('<input type="hidden" id="' + compID + '_hidden"  value="" /><div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-search"><span class="sr-only">검색</span></i></button></div>');
...
$.fn.telEditor = function(params) {
	var compID = $(this).attr("id");

	$("#" + compID).attr('readonly', true);
	$("#" + compID).val(params.txtVal);
	$("#" + compID).wrap('<div id="' + compID + '_wrap" class="input-group"></div>');
	if (gfn_str_parseNull(params.txtVal) != "" && gfn_str_parseNull(params.originVal) != "") {
		$("#" + compID).parent().append('<input type="hidden" id="' + compID + '_hidden"  value="' + params.originVal + '" /><div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-x"><span class="sr-only">삭제</span></i></button></div>');
	} else {
		$("#" + compID).parent().append('<input type="hidden" id="' + compID + '_hidden"  value="' + params.originVal + '" /><div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-pencil"><span class="sr-only">작성</span></i></button></div>');
	}

	$("#" + compID + "_wrap button").click(function(e) {
		if ($("#" + compID + "_wrap button .bi-pencil").length > 0) {
			gfn_com_TelEditorPop(compID, params);
		} else {
			$(this).find("i").removeClass("bi-x");
			$(this).find("i").addClass("bi-pencil");
			$(this).find("i span").text("작성");
			$("#" + compID).val("");
			$("#" + compID + "_hidden").val("");
		}
	});
	...
	if (typeof params.callbackFunc == "function") {
		params.callbackFunc();
	} else if (typeof params.callbackFunc == "string") {
		window[params.callbackFunc]();
	}
}

function gfn_com_TelEditorPop(compID, params) {

	var _blank = (gfn_str_parseNull($("#" + compID).val()) == "" && gfn_str_parseNull($("#" + compID + "_hidden").val()) == "") ? true : false;
	var nationSplit = gfn_str_parseNull($("#" + compID + "_hidden").val()).split("|");

	if (nationSplit != undefined && nationSplit.length == 1) {
		nationSplit = ("0000|" + nationSplit[0]).split("|");
	}
	if (_blank || (nationSplit != undefined && nationSplit.length == 2)) {

		var formatFlag = _blank ? "_new" : (nationSplit[0] != "+82") ? "_etc" : "_normal";
		var telnoSplit = _blank ? [] : nationSplit[1].split("-");

		if ((formatFlag == "_normal" && telnoSplit.length == 3) || formatFlag == "_etc" || _blank) {
			scwin["telEditorPop"] = {
				compid: compID,
				formatFlag: formatFlag,
				telnoGbn: params.telnoGbn != undefined ? params.telnoGbn : "A",
				nationNoGcd: _blank ? "+82" : nationSplit[0],
				etcTelno: _blank ? "" : nationSplit[1],
				normalTelno1: _blank ? "" : telnoSplit[0],
				normalTelno2: _blank ? "" : telnoSplit[1],
				normalTelno3: _blank ? "" : telnoSplit[2],
				title: $("#" + compID).closest(".b-row-item").find(".b-title-box label").text(),
			...
			gfn_com_modalPopup({
				...
				height: 220,
				target: null,
				title: $("#" + compID).closest(".b-row-item").find(".b-title-box label").text() + "편집",
				include: "/html/ost/core/popup/telNoPop.html"
			});
		}
	} else {
		alert("편집 가능한 포멧이 아닙니다.<br />(ex) +81|010-1234-1234 or 0000|1234");
		return;
	}
}
...
$.fn.emailEditor = function(params) {
	...
	$("#" + compID).attr('readonly', true);
	$("#" + compID).val(params.val);
	$("#" + compID).wrap('<div id="' + compID + '_wrap" class="input-group"></div>');
	if (gfn_str_parseNull(params.val) != "") {
		$("#" + compID).parent().append('<div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-x"><span class="sr-only">삭제</span></i></button></div>');
	} else {
		$("#" + compID).parent().append('<div class="input-group-append"><button type="button" class="btn form-control"><i class="bi bi-pencil"><span class="sr-only">작성</span></i></button></div>');
	}

	$("#" + compID + "_wrap button").click(function(e) {
		if ($("#" + compID + "_wrap button .bi-pencil").length > 0) {
			gfn_com_EmailEditorPop(compID, params);
		} else {
			$(this).find("i").removeClass("bi-x");
			$(this).find("i").addClass("bi-pencil");
			$(this).find("i span").text("작성");
...
function gfn_com_EmailEditorPop(compID, params) {

	var _blank = (gfn_str_parseNull($("#" + compID).val()) == "") ? true : false;
	var domainSplit = gfn_str_parseNull($("#" + compID).val()).split("@");

	if (_blank || (domainSplit != undefined && domainSplit.length == 2)) {
		scwin["emailEditorPop"] = {
			compid: compID,
			mailname: _blank ? "" : domainSplit[0],
			domain: _blank ? "pusan.ac.kr" : domainSplit[1],
			dataBound: params.dataBound
		}
		gfn_com_modalPopup({
			width: 750,
			...
			target: null,
			title: "이메일편집",
			include: "/html/ost/core/popup/emailPop.html"
		});
	} else {
		alert("편집 가능한 포멧이 아닙니다.<br />(ex) mailname@domain");
		return;
	}
}

function gfn_com_openJusoPop(params) {
	if (scwin.loginUser.userIp == "127.0.0.1" || scwin.loginUser.userIp == "0:0:0:0:0:0:0:1") {
		if (typeof params.callbackFunc == "function") {
			params.callbackFunc({ "zipCd": "46241", "addr": "부산 금정구 부산대학로63번길 2 (장전동, 부산대학교)", "addrEng": "pusan UNIV", "bdNm": "부산대학교" });
		} else if (typeof params.callbackFunc == "string") {
			window[params.callbackFunc]({ "zipCd": data.zonecode, "addr": fullAddr });
		}
	} else {
		scwin["jusoPop"] = {
		...
		gfn_com_modalPopup({
			...
			target: null,
			title: "주소찾기",
			include: "/html/ost/core/popup/addressPop.html"
		});
	}
}

function gfn_com_createSliderComp(compID,callbackFunc) {
	$("#" + compID).hide();
...
function gfn_com_createSliderComps(obj,callbackFunc) {
	setTimeout(function() {
		$.each($(obj), function(idx) {
			if (!gfn_com_isEmptyObject($(this).attr("id"))) {
				gfn_com_createSliderComp($(this).attr("id"),callbackFunc);
			}
		});
	}, 300);
}

function gfn_com_clausePop(_options) {
	scwin["clause"] = {
		_options: _options
	};
	gfn_com_modalPopup({
		width: 1000,
		height: 650,
		title: _options.title ? _options.title : "개인정보동의",
		scroll: "off",
		include: "/html/ost/core/popup/clausePop.html"
	});
}
...
alert2 = function(msg, callBack) {
	LayerPopup.alert2(msg, callBack);
}
...
fn_popupMultiFileViewr = function(down_url,_this) {
	if (scwin.loginUser.userIp == "127.0.0.1" || scwin.loginUser.userIp == "0:0:0:0:0:0:0:1") {
		alert("파일뷰어는 운영사이트에서만 지원 됩니다.",$(_this));
		return false;
	}
	...
	var fid = $('<input type="hidden" name="fid" value="SES_HOME_' + fn_getRandomKey() + '">');
...
$.fn.hasVerticalScrollBar_cont = function() {
		return this.get(0) ? this.innerHeight() > this.closest(".table-responsive").innerHeight()+2: false;
}
$.fn.hasHorizontalScrollBar_cont = function() {
		return this.get(0) ? this.innerWidth() > this.closest(".table-responsive").innerWidth()+2 : false;
}

$.fn.hasVerticalScrollBar = function() {
		return this.get(0) ? this.get(0).scrollHeight > this.innerHeight()+2: false;
}
$.fn.hasHorizontalScrollBar = function() {
        return this.get(0) ? this.get(0).scrollWidth > this.innerWidth() : false;
}
function loadModal(){
	//모달창이 열리면 body로 초점 이동
    	$(".modal").on("shown.bs.modal", function(){
    		$(".modal").attr("tabindex","0");
    			$(".modal-body").attr("tabindex","0");
    			$(".modal-body").focus();
    			$(".modal").focus(function(){
					$(".modal-body").removeAttr("tabindex");
    				$(".close").focus();	
    			});
    	});
}
...
function tagFocus(){
	//특정 태그에서 onclick 이벤트가 있는 태그는 초점 이동 가능하게 제어
    	$.each($("span, div, p"),function(i,o){
...
function accordionLabelChg(){
		/** 아코디언 card-header 카드 **/
		$.each($(".accordion .card button"), function() {
				if ($(this).attr("data-expanded") == "true" || $(this).attr("aria-expanded") == "true") {
					$(this).attr("title", "접기");
				} else {
					$(this).attr("title", "열기");
				}
		...
		/** 아코디언 card-body 카드 **/
		$.each($(".accordion .card-body a"), function() {
				if ($(this).attr("data-expanded") == "true" || $(this).attr("aria-expanded") == "true") {
					$(this).attr("title", "접기");
				} else {
					$(this).attr("title", "열기");
		...
		$(".accordion .card-body a").bind("click", function() {
			...
			if ($(this).attr("data-expanded") == "true" || $(this).attr("aria-expanded") == "true") {
				...
				setTimeout(function(){
					...
					$.each($("table"),function(i,o){
						if ($(o).hasVerticalScrollBar_cont() || $(o).hasHorizontalScrollBar_cont()) { 
							$(o).attr("tabindex",0);
						}
					});
				},300);
...
function onPopupOpenerClickFocus(){
	// 팝업 오픈 버튼을 클릭하면 클릭한 초점을 가지고 있는다.
    	//$("#main a,#main button,#header a,#header button ,table td").on("click",function (e){
		$("a, button").on("click",function (e){
			let class_nm = gfn_com_parseNull($(this).attr("class"));
			//close 버튼 or 창을 닫는 버튼은 여기로 오지 않게 한다.
    		if(class_nm.indexOf("close-btn")<0 && class_nm.indexOf("pop_close")<0 
				&& class_nm.indexOf("modal_alert_close")<0 
					&& gfn_com_parseNull($(this).parent().attr("class")).indexOf("attach-btn") < 0  ){ //&& $(this).attr("class").indexOf("btn-danger")<0
	    		let this_target = $(this);
					var target_index = $("[id^='target_href']").length +1;
	    			var target_href = "target_href"+target_index;
	    			if(this_target.attr("id") == undefined ){
	    				this_target.attr("id",target_href);
	    			}else{
	    				target_href = this_target.attr("id");
	    			}
	    			objTarget = this_target;

					var suffixStr = "";
					let pop_index = $("div[id^='popupWrap']").length ;
					for(var idx = 1; idx < pop_index; idx++) {
						suffixStr += "_";
					}
					setTimeout(function() {
					if( $(".alert_popup").css("top").replaceAll("px","") < 0
						&& $(".alert2_popup").css("top").replaceAll("px","") < 0
						&& $(".confirm_popup").css("top").replaceAll("px","") < 0){
						setTimeout(function() {
			    			$("#popupWrap"+suffixStr+" .pop_close").attr("href","#"+target_href);
				    		$("#popupWrap"+suffixStr+" .pop_close").on("click",function (e){
				    			if(this_target.attr("id") == "target_href"+target_index){
				    				this_target.focus();
				    				this_target.removeAttr("id");
				    			}else{
									this_target.focus();
								}
				        	});
						...
						setTimeout(function() {
			    			$(".modal_alert_close").attr("href","#"+target_href);
				    		$(".modal_alert_close").on("click",function (e){
				    			if(this_target.attr("id") == "target_href"+target_index){
				    				this_target.focus();
				    				this_target.removeAttr("id");
				    			}else{
	    		...
	    		/*}else{
	    			setTimeout(function() {
			    		$(".pop_close, .close-btn").on("click",function (e){
			        		$this.focus();
			        	});
		    		}, 300);
	    		}*/
        ...
        if(event.keyCode != 8) {
            ...
            if(obj.value.replace(/[0-9 \-,:]/g, "").length == 0) {
                ...
                if(number.length < 5) {
					...
					ymd += number.substr(8, 2);
					ymd += ":";
					ymd += number.substr(10,2);
					ymd += ":";
					ymd += number.substr(12);
				}
                obj.value = ymd;
				
            } else {
                alert("숫자 이외의 값은 입력하실 수 없습니다.",obj);
...
gfn_onlineManualJspPop = function(){
     var option = {
         url: '/manual/online/view'
         , name: 'manualJspPopup'
         , width: Math.ceil(window.outerWidth * 0.7)
    ...
    if(!option.toolbar) option.toolbar = "no";
    if(!option.menubar) option.menubar = "no";
    if(!option.location) option.location = "no";
    if(!option.scrollbars) option.scrollbars = "no";
    if(!option.status) option.status = "no";
    if(!option.resizable) option.resizable = "no";

    // 옵션 적용
    if(option.width) option_str += "width=" + option.width
    if(option.height) option_str += ", height=" + option.height
    if(option.toolbar) option_str += ", toolbar=" + option.toolbar
    if(option.menubar) option_str += ", menubar=" + option.menubar
    if(option.location)  option_str += ", location=" + option.location
    if(option.scrollbars) option_str += ", scrollbars=" + option.scrollbars
    if(option.status) option_str += ", status=" + option.status
    if(option.resizable) option_str += ", resizable=" + option.resizable
    if(option.fullscreen) option_str += ", fullscreen=" + option.fullscreen
    if(option.channelmode)   option_str += ", channelmode=" + option.channelmode
    if(option.left) option_str += ", left=" + option.left
    if(option.top) option_str += ", top=" + option.top
...
function tabFocusAccessibilityCtl(obj){
		var objId = "#"+obj;
		var owlItem = ".nav-item";
		if("ost_tabs1" == obj) owlItem = ".owl-item";
		if($(objId).find("#tablist").length == 0 ){
			return;
		}

		$.each($(objId+" .nav-link"), function() {
		...
		//컨텐츠 대체텍스트
		$(objId+" .nav-link").on("click", function() {
			$.each($(objId+" .nav-item"),function(){
		...
		$.each($(objId).find("#tablist").eq(0).find('a'),function(i,o){
			if(i != 0){
				$(this).before('<i class="linkTabMenuSt" tabindex="0" aria-hidden="true" ></i>');	
			}
			$(this).after('<i class="linkTabMenuEnd" tabindex="0" aria-hidden="true" ></i>');
		});
		$(objId).find(".tab-content:eq(0)").append('<i class="tabContLastSt"  tabindex="0" aria-hidden="true" ></i>');
		$(objId).find(".tab-content:eq(0)").append('<i class="tabContLastEnd" tabindex="0" aria-hidden="true" ></i>');
		$.each($(objId).find(".tab-content").eq(0).children('.tab-pane'),function(i,o){
			$(this).append('<i class="linkTabCont" tabindex="0" aria-hidden="true" ></i>');
			$(this).before('<i class="tabContFst"  tabindex="0" aria-hidden="true" ></i>');	
		});
		
		$(objId +" .linkTabMenuSt").focus(function(e){
				//if($(this).closest(".owl-item").prev("div").length == 0){
				//	$(this).closest(".sec-box-major").prev("div").find("a:eq(0)").focus();
				//}else{
					$(this).closest(owlItem).prev().find("a").focus();	
				//}
		...
		$(objId+" .linkTabMenuEnd").focus(function(e){
			var thisIndex = $(objId+" .linkTabMenuEnd").index(this);
			var linkType = "";
			if("meal_tabs" == obj ){
				linkType="button"
			}else{
				...
				if($(this).parent().find("a").hasClass("active")){
					if($(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("a").length ==0
						&& $(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("button").length ==0){ //내용에 a링크가 없으면
						if($(this).closest(owlItem).next().find("a").length == 0){
							$(objId+" .tabContLastSt").focus();
						}else{
							$(this).closest(owlItem).next().find("a").focus();
						}
					}else{
						var tabcontentHtml = $(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).html();
						if(tabcontentHtml.indexOf("<button") >0 && tabcontentHtml.indexOf("<a") >0 ){
							if(tabcontentHtml.indexOf("<button") > tabcontentHtml.indexOf("<a")){
								if("meal_tabs" == obj ){
									$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find(".active").find("a:eq(0)").focus();	
								}else{
									$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("a:eq(0)").focus();	
								}
							}else{
								$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("button:eq(0)").focus();	
							}
						}else{
							if(tabcontentHtml.indexOf("<button") >0){
								$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("button:eq(0)").focus();
							}else{
								$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find("a:eq(0)").focus();
							}
						...
						//$(objId).find(".tab-content:eq(0)").children(".tab-pane").eq(thisIndex).find(linkType+":eq(0)").focus();	
					}
				}else{
					if($(this).closest(owlItem).next().length == 0){
						$(objId+" .tabContLastSt").focus();
					}else{
						$(this).closest(owlItem).next().find("a").focus();	
					}
		...
		$(objId +" .tabContFst").focus(function(e){	
			var thisIndex = $(objId+" .tabContFst").index(this);
			$(objId).find("#tablist").find('a').eq(thisIndex).focus();
		});
		
		$(objId+" .tabContLastSt").focus(function(e){	
			if("#ost_tabs1" == objId){
				$(this).closest(".sec-box-major").next("div").find("a:eq(0)").focus();	
			}else if("#meal_tabs" == objId){
				$(objId).next().find("a:eq(0)").focus();	
			}else{

				if($(objId).next("div").length > 0){
					$(objId).next("div").find("select:eq(0)").focus();	
				}else{
					$(".container-sub").next().find("a:eq(0)").focus();
				}
				
				
				/*
				if($(objId).next(".sec-body").find("a").length >0){
					$(objId).next(".sec-body").find("a:eq(0)").focus();	
				}
				$(objId).next(".sec-body").find("a:eq(0) ,select:eq(0)").focus();
				*/	
			}
		});
		$(objId+" .tabContLastEnd").focus(function(e){
			var lastIndex = $(objId).find("#tablist").find('a').length-1;
			$(objId).find("#tablist").find('a').eq(lastIndex).focus();
		});

		$(objId+" .linkTabCont").focus(function(e){	
			var thisIndex = $(objId+" .linkTabCont").index(this);
			if(thisIndex == $(objId+" .linkTabCont").length-1){
				$(objId+" .tabContLastSt").focus();
			}else{
				$(objId).find("#tablist").find('a').eq(thisIndex+1).focus();
			}
