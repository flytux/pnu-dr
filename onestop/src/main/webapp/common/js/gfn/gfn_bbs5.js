...
$.fn.bbsListView = function(params) {
		...
		$.each(params.bbsDataList, function(index, item) {
			...
			switch (objId) {
				...
				case "board-webzine":
					...
					data += "</h4>";
					data += "			<p class='txt'>" + item.CONTENT + "</p>";
				...
				case "board-faq":
					...
					data += "		<button class='btn btn-block text-left " + (index != 0 ? "collapsed" : "") + "' type='button' data-toggle='collapse' data-target='#collapse" + index + "' data-expanded='" + (index == 0 ? "true" : "false") + "' data-controls='collapse" + index + "'>" + item.TITLE_CONTENT + "</button>";
					data += "	</div>";
	...
	if (params.searchData) {
		...
		headerInfoStr += '		<option value="INS_USER_ID" data-lang>아이디</option>';
	...
	if (params.bbsMgrAuth.WRITEOUT_AUTH_FG == "Y" && scwin.loginUser.userId !="tst601200") {  //2023-09-15 접금성심사용 id 이면 권한 없음 
		var btnBoxStr = '';
