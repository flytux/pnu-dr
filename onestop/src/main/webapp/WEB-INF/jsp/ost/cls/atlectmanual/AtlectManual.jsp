	...
	<main id="main">
		<section class="sec-1">
			...
			<div class="sec-body">
				...
				<div class="search-box">
					<div class="b-table-box flex-col-3" role="table" aria-label="조회">
						...
						<div class="b-row-box" role="row" id="hakGbn">
							...
							<div class="b-row-item">
								<div class="b-title-box" role="rowheader">
									<label id="deptMajorCds" for="SCH_DEPT_CD_multi" class="" data-lang>학과(부)/전공</label>
								</div>
								<div class="b-con-box" role="cell">
						...
						<div class="b-row-box" role="row" id="searchGbn">
							<div class="b-row-item merge-3">
								...
								<div class="b-con-box" role="cell">
									<input type="radio" id="SEARCH_GBN1" name="SEARCH_GBN" class="" style="" value="1" checked="checked"/><label for="SEARCH_GBN1" class="" data-lang>세부구분(영역별, 학과별)</label>
									<div style="margin-left: 20px;"></div>
									<input type="radio" id="SEARCH_GBN2" name="SEARCH_GBN" class="" style="" value="2"/><label for="SEARCH_GBN2" class="" data-lang>교과목명 직접입력</label>
								</div>
							</div>
						</div>
				...
				<div class="message-box" id="message_lang">
					<div class="message-box-inner">
						<div class="message-body">
							<ul class="list list-style-1">
...
<!-- 									<th scope="col"><span style="font-size:12px"><label data-lang>수강<br/>대상학과</label></span></th> -->
...
<script type="text/x-tbody-template" id="resultTemplate_kor">
	<tr style="cursor:pointer;">
		...
		<td style="font-size:12px">#:ALLOC_RCNT#
	        ...
	        #} else if ( :COLL_GRAD_GCD == "0002" && :COURSE_COLL_GRAD_GCD == "0001") { #
	               	<button class="btn btn-outline-secondary rcnt-detail" type="button"><i class="bi bi-display" aria-hidden="true"></i><a style="font-size:13px;">상세</a>
					</button>	               
	        #} else { #
...
<script type="text/x-tbody-template" id="resultTemplate_eng">
	<tr style="cursor:pointer;">
		<td>#:RN#</td>
		<td class="text-left" style="font-size:12px">#:MNG_DEPT_ENG_NM#</td> 
        <td style="font-size:12px">#:ALL_CYBER#</td>
        <td style="font-size:12px">#:NATIVE_LANG_NM#</td>
	    <td style="font-size:12px">#:STDT_YEAR_NM#</td> 
	    <td style="font-size:12px">#:SUBJ_GCD_ENG_NM#</td>
	    <td class="text-left" ><a class="subj-detail" style="color:blue; font-size:12px" href ="#popup">#:SUBJ_ENG_NM#</a></td> 
		<td style="font-size:12px">#:SUBJ_NO#</td> 
		<td style="font-size:12px">#:CLASS_NO#<br/>#:CRDT#/#:SIGAN#</td> 
		<td style="font-size:12px">#:ALLOC_RCNT#
			# if( :COLL_GRAD_GCD == "0001") { #
		...
		<td class="text-left"><a class="prof-detail" style="color:blue; font-size:12px;" href ="#popup">#:PROF_ENG_NM#</a></td> 
		<td class="text-left" style="font-size:12px">#:TIMETABLE_SUMMARY_INFO#</td> 
		<!--<td class="text-left" style="font-size:12px">#:ATLECT_DEPT_NM#</td>--> 
		
		<td>	
			# if( :PRT_KOR == "국문") { #
	               <button class="btn btn-outline-primary kor" type="button" ><a style="font-size:5px;">KOR</a></button>
	        #} else if ( :PRT_KOR == "") { #
	               
	        #}#
			# if( :PRT_ENG == "영문") { #
	               <button class="btn btn-outline-secondary eng" type="button"><a style="font-size:5px;">ENG</a></button>
	        #} else if ( :PRT_KOR == "") { #
...
<script type="text/javascript">
	...
	if($('#langGcd').val()=="0001"){
		fn_excelDownNm = "fn_excelDownload";
		resultTemp_str = $("#resultTemplate_kor");
		sch_All = "전체";
		sch_Select = "선택";
		$("#message_lang").show();
		...
		fn_excelDownNm = "fn_excelDownloadEng";
		resultTemp_str = $("#resultTemplate_eng");
		$("#message_lang").hide();
		sch_All = "All";
		sch_Select = "Select";
		...
		var subjGcdJson1 = [ {
			...
			CODE : 4,
			LABEL : 'Required General Education Courses'
		}, {
			CODE : 2,
			LABEL : 'Fundamental Major & First-year students'
		}, {
			CODE : 1,
			LABEL : 'Major/Teaching Courses (2nd, 3rd, 4th grade)'
		} ];
		var subjGcdJson2 = [ {
			CODE : 3,
			...
			CODE : 2,
			LABEL : 'Informatization Literacy, Required Fundamental Courese'
		}, {
			CODE : 1,
			LABEL : 'Major/Teaching courses(2nd,3rd,4th grade)'
		} ];
	...
	function fn_VisibleSet(info) {
		if (info) { //조회시 필수체크 변경
			...
			if ($("#SCH_TERM_GCD").val() == "0010"
					|| $("#SCH_TERM_GCD").val() == "0020"|| $("#SCH_TERM_GCD").val() == "0011"|| $("#SCH_TERM_GCD").val() == "0021") { //정규학기
			...
			}/*  else if ($("#SCH_TERM_GCD").val() == "0011"				--2023.01.09 계절학기도 정규학기와 동일하게 검색조건 변경 (아이디노 김동영)
			...
			if ($("#SCH_TERM_GCD").val() == "0010"
					|| $("#SCH_TERM_GCD").val() == "0020" || $("#SCH_TERM_GCD").val() == "0011"
						|| $("#SCH_TERM_GCD").val() == "0021") { //정규학기
	...
	function fn_openReport(param,gbn) {
        ...
        if(gfn_str_parseNull(param.EDUCERT_GCD) == ""){ //교육인증 구분코드 없을때./////////////////////////////////////////////////////////////////////////////
        	...
        	objParam.SCH_SYEAR = paramData.SCH_SYEAR;
			...
			success : function(data, responseData) {
				if (data.length > 0) {
					...
					gfn_com_excelDownloadFile([ {
						...
						key : "NATIVE_LANG_NM",
						name : "Courses in foreign languages"
					}, {
						key : "STDT_YEAR_NM",
						name : "Grade"
					}, {
						...
						key : "SUBJ_ENG_NM",
						name : "Course title (undecided division)"
					}, {
						key : "SUBJ_NO",
						name : "Course number"
					}, {
						...
						key : "ALLOC_RCNT",
						name : "The number of students to be registered	"
					}, {
						key : "PROF_ENG_NM",
						name : "Professor name"
					}, {
						...
						key : "ATLECT_DEPT_NM",
						name : "Department for taking course"
					}, {
						key : "MNG_DEPT_ENG_NM",
						name : "Department"
					}, {
					...
					data, /* 엑셀로 다운로드 할 데이터 */
					"Course catalog" /* 엑셀로 다운로드 할 파일명 (확장자제외) */
					);
				} else {
					alert("엑셀로 다운로드할 데이터가 없습니다.");
					return false;
